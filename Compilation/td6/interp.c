...
{ if(e != NULL)
	...
	{
	 case Ind:
	   { pos = rechty(e->FG->ETIQ, rho_gb.debut);
	    res = TAS[ADR[pos->VAL] + semval(rho_gb, e->FD)];
	     return(res);/* a ecrire */
	   }
	...
	case NewAr:                     /*creation tableau */
	  {//La taille est sur le fils droit (choix unitaire)
...
{char *lhs; int rhs; int cond;
 if(c != NULL)
	...
	case Se: 
	  ...
	  break; 
       case If:/* a ecrire */ //DONE
	  cond = semval(rho_gb, c->FG);
	  if (cond){
	    semop_gp(rho_gb, c->FD);
	  }
	  break;
       case Wh:/* a ecrire */  //DONE
	  cond = semval(rho_gb,c->FG);
	  if(cond){
	    semop_gp(rho_gb, c->FD);
	    semop_gp(rho_gb, c);
